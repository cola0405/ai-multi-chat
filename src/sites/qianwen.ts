 ...
 *
 * 通用入口:
 *   npx tsx src/run.ts --site qianwen "你好"
 */
...
用法:
  npx tsx src/sites/qianwen.ts "提示词"
  npx tsx src/sites/qianwen.ts "提示词" --file ./image.png
  npx tsx src/sites/qianwen.ts --discover
...
`);
  ...
  c.setSession(process.env.PW_SESSION || "qianwen");
  if (!process.env.SKIP_ATTACH) {
    console.log("[qianwen] 正在连接 Chrome (playwright-cli attach)...");
    c.attach();
    console.log("[qianwen] Chrome 已连接");
  } else {
    console.log("[qianwen] Chrome 已由主进程连接，跳过 attach");
  }
