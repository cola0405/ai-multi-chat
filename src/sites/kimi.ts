...
// ─── playwright-cli 封装 ────────────────────────────────
const SESSION = process.env.PW_SESSION || "kimi";
...
const KW = {
  input: ["textbox", "textarea", "contenteditable", "输入", "聊天", "输入框", "prompt", "message", "请输入", "问点什么"],
  send: ["发送", "send", "submit", "提交", "arrow-up", "ArrowUp"],
  newChat: ["新对话", "新聊天", "new chat", "new-chat", "新建对话"],
};

function log(...args: unknown[]) { console.log("[kimi]", ...args); }

// ─── 上传附件（drop 事件 → contenteditable）──────────
...
async function main() {
  ...
  if (args.length === 0) {
    console.log(`用法: npx tsx src/sites/kimi.ts "提示词" [--file ./img.png]`);
    process.exit(0);
  }
