 ...
 *
 * 通用入口:
 *   npx tsx src/run.ts --site chatglm "你好"
 */
...
用法:
  npx tsx src/sites/chatglm.ts "提示词"
  npx tsx src/sites/chatglm.ts "提示词" --file ./image.png
  npx tsx src/sites/chatglm.ts --discover
...
`);
  ...
  c.setSession(process.env.PW_SESSION || "chatglm");
  if (!process.env.SKIP_ATTACH) {
    console.log("[chatglm] 正在连接 Chrome (playwright-cli attach)...");
    c.attach();
    console.log("[chatglm] Chrome 已连接");
  } else {
    console.log("[chatglm] Chrome 已由主进程连接，跳过 attach");
  }
