/**
 * 腾讯元宝 (yuanbao.tencent.com) 站点脚本
 *
 * 独立运行:
 *   npx tsx src/sites/yuanbao.ts "你好"
 *   npx tsx src/sites/yuanbao.ts "分析图片" --file ./test.png
 *
 * 通用入口:
 *   npx tsx src/run.ts --site yuanbao "你好"
 */
...
用法:
  npx tsx src/sites/yuanbao.ts "提示词"
  npx tsx src/sites/yuanbao.ts "提示词" --file ./image.png
  npx tsx src/sites/yuanbao.ts --discover
...
`);
  ...
  c.setSession(process.env.PW_SESSION || "yuanbao");
  if (!process.env.SKIP_ATTACH) {
    console.log("[yuanbao] 正在连接 Chrome (playwright-cli attach)...");
    c.attach();
    console.log("[yuanbao] Chrome 已连接");
  } else {
    console.log("[yuanbao] Chrome 已由主进程连接，跳过 attach");
  }
